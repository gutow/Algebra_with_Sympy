...
attempting to isolate (solve for) a variable. Thus the statement ``Equation/b`` yields a new equation
...
_Setup/Installation_: Currently this tool is not available as a pip installable package. The file
``algebraic_equation.py`` must be available for import in the directory space of the active Python, IPython or Jupyter
notebook. To activate issue the command: ``from algebraic_equation import *``. This will also import the SymPy tools.
If you want to isolate this tool from the global namespace you are working with change the import statement to ``import
algebraic_equation as spa``, where ``spa`` stands for "SymPy Algebra". Then all calls would be made to
``spa.funcname()``.

Usage examples can be found in the docstrings and the demonstration Jupyter notebook
``Demonstration of algebraic_equation.py.ipynb``.
...
class Equation(Basic):
    """
    This class defines an equation with a left-hand-side (lhs) and a right-hand-side (rhs) connected by an operator
    (e.g. $p*V = n*R*T$).
    ...
    def __new__(cls, lhs, rhs, **kwargs):
        ...
        if check:
            ...
            if lhs.is_number and rhs.is_number and lsimp != rsimp:
                ...
                warn('Did your really mean to define unequal numbers ' + str(lsimp) + ' and ' + str(
                    rsimp) +
                     ' as equal? To suppress this warning include `check=False` in the equation definition: ' +
        ...
        if side == 'both':
            if (hasattr(self.lhs, str(func))) or (hasattr(self.rhs, str(func))):
                return Equation(getattr(self.lhs, str(func))(*args, **kwargs),
                                getattr(self.rhs, str(func))(*args, **kwargs), check=False)
            else:
    ...
    def _latex(self, obj, **kwargs):
        return latex(self.lhs, **kwargs) + self.relop + latex(self.rhs, **kwargs)
